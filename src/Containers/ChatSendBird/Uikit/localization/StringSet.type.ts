...
export interface StringSet {
  ...
  GROUP_CHANNEL_LIST: {
    ...
    CHANNEL_PREVIEW_TITLE: (
      currentUserId: string,
      channel: SendbirdGroupChannel,
    ) => string
    CHANNEL_PREVIEW_TITLE_CAPTION: (
      channel: SendbirdGroupChannel,
      locale?: Locale,
    ) => string
...
export const createBaseStringSet = ({
  dateLocale,
  overrides,
}: StringSetCreateOptions): StringSet => {
  const USER_NO_NAME = overrides?.LABELS?.USER_NO_NAME ?? '(No name)'
  const CHANNEL_NO_MEMBERS =
    overrides?.LABELS?.CHANNEL_NO_MEMBERS ?? '(No members)'
  return {
    GROUP_CHANNEL: {
      ...
      LIST_DATE_SEPARATOR: (date, locale) =>
        dateSeparator(date, locale ?? dateLocale),
      LIST_BUTTON_NEW_MSG: newMessages => `${newMessages.length} new messages`,

      MESSAGE_BUBBLE_TIME: (message, locale) =>
        messageTime(new Date(message.createdAt), locale ?? dateLocale),
    ...
    GROUP_CHANNEL_LIST: {
      ...
      CHANNEL_PREVIEW_TITLE_CAPTION: (channel, locale) =>
        getGroupChannelPreviewTime(channel, locale ?? dateLocale),
