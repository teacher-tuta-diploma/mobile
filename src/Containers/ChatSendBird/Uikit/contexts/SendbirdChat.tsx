...
export const SendbirdChatProvider = ({
  ...
  const updateCurrentUserInfo: Context['updateCurrentUserInfo'] = useCallback(
    async (nickname, profile) => {
      ...
      if (!user) {
        throw new Error(
          'Current user is not defined, please connect using `useConnection()` hook first',
        )
      }
      ...
      if (typeof profile === 'string') {
        ...
        throw new Error(
          `Cannot update profile, not supported profile type(${typeof profile})`,
        )
      }
  ...
  const markAsDeliveredWithChannel: Context['markAsDeliveredWithChannel'] =
    useCallback(
      (channel: SendbirdGroupChannel) => {
        if (appFeatures.deliveryReceiptEnabled) {
          confirmAndMarkAsDelivered(sdkInstance, channel)
        }
      },
  ...
  useEffect(() => {
    const listener = (status: AppStateStatus) => {
      ...
      if (status === 'active') {
        sdkInstance.connectionState === 'CLOSED' &&
          sdkInstance.setForegroundState()
      } else {
        sdkInstance.connectionState === 'OPEN' &&
          sdkInstance.setBackgroundState()
      }
    }
  ...
  const value: Context = {
    ...
    features: {
      ...
      channelListMessageReceiptStatusEnabled:
        enableChannelListMessageReceiptStatus,
    },
  }
