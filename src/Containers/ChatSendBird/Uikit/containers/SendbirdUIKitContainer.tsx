...
const SendbirdUIKitContainer = ({
    ...
    if (NetInfo?.addEventListener) {
      const listener = (
        ...
        const unsubscribe = NetInfo.addEventListener(state => {
          ...
          if (online && callbackType === 'online') {
            callback()
          }
          if (!online && callbackType === 'offline') {
            callback()
          }
        })
  ...
  const [sdkInstance, setSdkInstance] =
    useState<SendbirdChatSDK>(getSendbirdSDK)
    ...
    <SafeAreaProvider>
      <SendbirdChatProvider
        ...
        enableAutoPushTokenRegistration={
          chatOptions?.enableAutoPushTokenRegistration ?? true
        }
        enableChannelListTypingIndicator={
          chatOptions?.enableChannelListTypingIndicator ?? false
        }
        enableChannelListMessageReceiptStatus={
          chatOptions?.enableChannelListMessageReceiptStatus ?? false
        }
      >
        <LocalizationProvider
          stringSet={localization?.stringSet ?? StringSetEn}
        >
