...
const GroupChannelListList = ({
  ...
  const onLongPress = useFreshCallback((channel: SendbirdGroupChannel) => {
    ...
    const menuItem = menuItemCreator({
      title: STRINGS.GROUP_CHANNEL_LIST.DIALOG_CHANNEL_TITLE(
        currentUser?.userId ?? '',
        channel,
      ),
      menuItems: [
        {
          title:
            STRINGS.GROUP_CHANNEL_LIST.DIALOG_CHANNEL_NOTIFICATION(channel),
          onPress: async () => {
            if (action === 'on') {
          ...
          onError: () => {
            toast.show(
              action === 'on'
                ? STRINGS.TOAST.TURN_ON_NOTIFICATIONS_ERROR
                : STRINGS.TOAST.TURN_OFF_NOTIFICATIONS_ERROR,
              'error',
            )
          },
        },
        {
          ...
          onPress: async () => {
            channel
              .leave()
              .then(() => sdk.clearCachedMessages([channel.url]).catch())
          },
