...
const createGroupChannelInviteFragment = <UserType,>(
  ...
  return ({
    ...
    const { users, refreshing, refresh, next, error, loading } = useUserList(
      sdk,
      {
        queryCreator,
        sortComparator,
    ...
    const _renderUser: NonNullable<typeof renderUser> = useCallback(
      (user, selectedUsers, setSelectedUsers) => {
        ...
        const sbSetSelectedUsers =
          setSelectedUsers as unknown as React.Dispatch<
            React.SetStateAction<SendbirdUser[]>
          >

        const joinedUserIds = activeChannel.members.map(u => u.userId)
        const userIdx = sbSelectedUsers.findIndex(
          u => u.userId === sbUser.userId,
        )
    ...
    return (
      <UserListModule.Provider
        headerRight={selectedUsers =>
          STRINGS.GROUP_CHANNEL_INVITE.HEADER_RIGHT({ selectedUsers })
        }
      ...
      >
        ...
        <StatusComposition
          ...
          ErrorComponent={
            <UserListModule.StatusError onPressRetry={() => refresh()} />
          }
        >
          <UserListModule.List
            onLoadNext={next}
