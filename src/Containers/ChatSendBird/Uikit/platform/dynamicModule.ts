...
const SBUDynamicModule = {
  ...
  get<K extends SBUNativeModule, T extends DynamicModules[K]>(
    name: K,
    logLevel: 'error' | 'warn' | 'none' = 'error',
  ) {
    const dmi = this.getInfo(name)
    if (!dmi) {
      Logger.log(`${name} doesn't exist in the dynamic module`)
    }
