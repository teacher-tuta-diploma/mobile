...
const ChatFlatList = forwardRef<ChatFlatListRef, Props>(function CustomFlatList(
  ...
  const _onScroll = useCallback<NonNullable<Props['onScroll']>>(
    event => {
      ...
      if (
        BOTTOM_DETECT_THRESHOLD < yPos.current &&
        contentOffset.y <= BOTTOM_DETECT_THRESHOLD
      ) {
        onLeaveScrollBottom(false)
      } else if (
        BOTTOM_DETECT_THRESHOLD < contentOffset.y &&
        yPos.current <= BOTTOM_DETECT_THRESHOLD
      ) {
    ...
    [onScroll, onLeaveScrollBottom, onBottomReached],
  )
