  ...
  switch (action.type) {
    ...
    case 'update_channels': {
      getGroupChannels(action.value.channels).forEach(freshChannel => {
        const idx = draft.groupChannels.findIndex(
          staleChannel => staleChannel.url === freshChannel.url,
        )
        if (idx > -1) {
      ...
      compareByOrder &&
        (draft.groupChannels = draft.groupChannels.sort(compareByOrder))
      break
    }
    ...
    case 'set_channels': {
      if (action.value.clearPrev) {
        ...
        draft.groupChannels = [
          ...draft.groupChannels,
          ...getGroupChannels(action.value.channels),
        ]
      }
...
export const useGroupChannelListReducer = (order?: Order) => {
  const [{ loading, refreshing, groupChannels }, dispatch] = useReducer(
    defaultReducer,
    {
      loading: true,
      refreshing: false,
...
const createCompareByOrder = (order?: Order) => {
  ...
  return (
    channel1: SendbirdGroupChannel,
    channel2: SendbirdGroupChannel,
  ): number => {
    switch (order) {
